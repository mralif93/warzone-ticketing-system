...
<div class="min-h-screen bg-wwc-neutral-50">
    ...
    <div class="px-6 py-6">
        <div class="mx-auto">
            ...
            <!-- Revenue Verification Debug Panel -->
            @php
                // Calculate revenue from different sources for verification
                $dateFromFilter = request('date_from', '2020-01-01');
                $dateToFilter = request('date_to', now()->format('Y-m-d'));

                // 1. From PurchaseTicket (current method - ticket price_paid)
                $revenueFromTickets = \App\Models\PurchaseTicket::whereIn('status', ['sold', 'active', 'scanned'])
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('price_paid');

                // 2. From Orders (total_amount where paid)
                $revenueFromOrders = \App\Models\Order::where('status', 'paid')
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('total_amount');

                // 3. From Payments (amount where succeeded) - This should match Stripe
                $revenueFromPayments = \App\Models\Payment::where('status', 'succeeded')
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('amount');

                // 4. Stripe-only payments
                $stripeRevenue = \App\Models\Payment::where('status', 'succeeded')
                    ->whereNotNull('stripe_payment_intent_id')
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('amount');

                // 5. Online Banking payments
                $onlineBankingRevenue = \App\Models\Payment::where('status', 'succeeded')
                    ->where('method', 'online_banking')
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('amount');

                // 6. Refunded amount
                $refundedAmount = \App\Models\Payment::whereIn('status', ['refunded', 'partially_refunded'])
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
                    ->sum('refund_amount');

                // Get payment method breakdown
                $paymentBreakdown = \App\Models\Payment::where('status', 'succeeded')
                    ->whereBetween('created_at', [$dateFromFilter . ' 00:00:00', $dateToFilter . ' 23:59:59'])
            ...
            <div class="bg-yellow-50 border border-yellow-300 rounded-2xl shadow-sm mb-6 p-6">
                <h3 class="text-lg font-bold text-yellow-800 mb-4 flex items-center">
                    <i class='bx bx-bug text-xl mr-2'></i>
                    💰 Revenue Verification ({{ $dateFromFilter }} to {{ $dateToFilter }})
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                        <div class="text-xs text-gray-500 mb-1">From Tickets (price_paid)</div>
                        <div class="text-xl font-bold text-gray-800">RM {{ number_format($revenueFromTickets, 2) }}</div>
                        <div class="text-xs text-gray-400">Current report method</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-yellow-200">
                        <div class="text-xs text-gray-500 mb-1">From Orders (total_amount)</div>
                        <div class="text-xl font-bold text-gray-800">RM {{ number_format($revenueFromOrders, 2) }}</div>
                        <div class="text-xs text-gray-400">Paid orders only</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-green-200 bg-green-50">
                        <div class="text-xs text-green-600 mb-1">From Payments (succeeded)</div>
                        <div class="text-xl font-bold text-green-700">RM {{ number_format($revenueFromPayments, 2) }}</div>
                        <div class="text-xs text-green-500">✅ Should match payment gateways</div>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-white rounded-lg p-4 border border-blue-200">
                        <div class="text-xs text-blue-500 mb-1">Stripe Only</div>
                        <div class="text-xl font-bold text-blue-700">RM {{ number_format($stripeRevenue, 2) }}</div>
                        <div class="text-xs text-blue-400">Has stripe_payment_intent_id</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-purple-200">
                        <div class="text-xs text-purple-500 mb-1">Online Banking</div>
                        <div class="text-xl font-bold text-purple-700">RM {{ number_format($onlineBankingRevenue, 2) }}</div>
                        <div class="text-xs text-purple-400">method = online_banking</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 border border-red-200">
                        <div class="text-xs text-red-500 mb-1">Refunded</div>
                        <div class="text-xl font-bold text-red-700">RM {{ number_format($refundedAmount, 2) }}</div>
                        <div class="text-xs text-red-400">Total refunds processed</div>
                    </div>
                </div>

                <div class="bg-white rounded-lg p-4 border border-gray-200">
                    <div class="text-sm font-semibold text-gray-700 mb-2">Payment Method Breakdown (Succeeded):</div>
                        ...
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="border-b">
                                    ...
                                    <th class="text-right py-2 px-3">Total (RM)</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($paymentBreakdown as $pb)
                                <tr class="border-b">
                                    <td class="py-2 px-3">{{ $pb->method ?? 'Unknown' }}</td>
                                    <td class="text-right py-2 px-3">{{ $pb->count }}</td>
                                    <td class="text-right py-2 px-3 font-semibold">{{ number_format($pb->total, 2) }}</td>
                                </tr>
                                @endforeach
                                <tr class="bg-gray-50 font-bold">
                                    <td class="py-2 px-3">TOTAL</td>
                                    <td class="text-right py-2 px-3">{{ $paymentBreakdown->sum('count') }}</td>
                                    <td class="text-right py-2 px-3">{{ number_format($paymentBreakdown->sum('total'), 2) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="mt-4 text-xs text-yellow-700">
                    <strong>Note:</strong> The "From Payments (succeeded)" value should match your Stripe dashboard + Online Banking totals.
                    If there's a difference, it could be due to: pending payments, failed payments counted, or refunds not deducted.
                </div>
            </div>
