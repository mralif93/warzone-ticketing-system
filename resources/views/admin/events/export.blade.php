...
@section('title', 'Export / Import Events')
    ...
    <div class="max-w-6xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-wwc-neutral-200">
            <div class="px-6 py-4 border-b border-wwc-neutral-100 flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-wwc-neutral-900">Export / Import Events</h1>
                    <p class="text-sm text-wwc-neutral-600">Choose fields with checkboxes. Leave all checked to export everything.</p>
                </div>
                <a href="{{ route('admin.events.index') }}" class="text-sm text-wwc-primary font-semibold hover:underline">← Back to Events</a>
            </div>
            <div class="p-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                <form method="GET" action="{{ route('admin.events.export') }}" class="space-y-4">
                ...
                <form method="POST" action="{{ route('admin.events.import') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
