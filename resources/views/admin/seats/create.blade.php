...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-wwc-neutral-900 font-display">Create New Seat</h1>
            <p class="text-wwc-neutral-600 mt-1">Add a new seat to the venue</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.seats.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="seat_identifier" id="seat_identifier" value="{{ old('seat_identifier') }}" required
                ...
                <div>
                    ...
                    <select name="price_zone" id="price_zone" required
                        ...
                        @foreach($priceZones as $zone)
                            <option value="{{ $zone->name }}" {{ old('price_zone') == $zone->name ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="seat_type" id="seat_type" required
                        ...
                        <option value="Box" {{ old('seat_type') == 'Box' ? 'selected' : '' }}>Box</option>
                        <option value="Standing" {{ old('seat_type') == 'Standing' ? 'selected' : '' }}>Standing</option>
                    </select>
                    @error('seat_type')
                        <p class="mt-1 text-sm text-wwc-error">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="base_price" id="base_price" step="0.01" min="0" value="{{ old('base_price') }}" required
